...
bool Address::Lookup(std::vector<Address::ptr>& rt, const std::string& host,
    ...
    if(error){
        EXHY_LOG_DEBUG(g_logger) << "Address::Lookup getaddress(" << host << ", "
    ...
    next = results;
    while(next){
        rt.push_back(Create(next->ai_addr,(socklen_t)next->ai_addrlen));
        next = next->ai_next;
    }
...
bool Address::GetInterfaceAddresses(std::multimap<std::string
    ...
    if(getifaddrs(&results) != 0) {
        EXHY_LOG_DEBUG(g_logger) << "Address::GetInterfaceAddresses getifaddrs "
    ...
    try{
        ...
        EXHY_LOG_ERROR(g_logger) << "Address::GetInterfaceAddresses exception";
        freeifaddrs(results);
        return false;
    }
    freeifaddrs(results);
...
IPAddress::ptr IPAddress::Create(const char* address, uint16_t port){
    ...
    if(error) {
        EXHY_LOG_ERROR(g_logger) << "IPAddress::Create(" << address
            << ", " << port << ") error=" << error
...
IPv4Address::ptr IPv4Address::Create(const char* address, uint32_t port){
    ...
    if(res<0){
        EXHY_LOG_INFO(g_logger)<<"Create error!";
        return nullptr;
    }
    return rt;
}
...
IPv6Address::ptr IPv6Address::Create(const char* address, uint16_t port){
    ...
    if(result <= 0) {
        EXHY_LOG_DEBUG(g_logger) << "IPv6Address::Create(" << address << ", "
