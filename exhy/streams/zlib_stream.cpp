...
int ZlibStream::init(Type type, int level, int window_bits
    ...
    EXHY_ASSERT((level >= 0 && level <= 9) || level == DEFAULT_COMPRESSION);
    EXHY_ASSERT((window_bits >= 8 && window_bits <= 15));
    EXHY_ASSERT((memlevel >= 1 && memlevel <= 9));
...
exhy::ByteArray::ptr ZlibStream::getByteArray() {
