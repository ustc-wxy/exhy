...
#ifndef __EXHY_CONFIG_H__
#define __EXHY_CONFIG_H__
...
//            return boost::lexical_cast<std::string>(m_val);
            ...
            EXHY_LOG_ERROR(EXHY_LOG_ROOT())<<"ConfigVar::toString exception"<<e.what()<<"convert: "<<typeid(m_val).name()<<" to string";
        }
        return "";
    }
...
//            m_val = boost::lexical_cast<T>(val);
            ...
            EXHY_LOG_ERROR(EXHY_LOG_ROOT())<<"ConfigVar::toString exception"<<e.what()<<"convert: string to "<<typeid(m_val).name();
        }
        return false;
    }
...
private:
    T m_val;
    RWMutexType m_mutex;
    //change call back funtions array
...
public:
    ...
    static typename ConfigVar<T>::ptr Lookup(const std::string& name,const T& default_value,
        ...
        if(it != GetDatas().end()){
            ...
            if(tmp){
                EXHY_LOG_INFO(EXHY_LOG_ROOT())<< "Lookup name="<<name<<"exists";
                return tmp;
            }else{
                EXHY_LOG_ERROR(EXHY_LOG_ROOT())<< "Lookup name="<<name<<"exists but type not "<<typeid(T).name()<<" real_type = "<< it->second->getTypeName()<<" "<<it->second->toString();
                return nullptr;
            }
        }
        ...
        if(name.find_first_not_of("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ._0123456789") != std::string::npos){
            EXHY_LOG_ERROR(EXHY_LOG_ROOT())<<"Lookup name invalid "<<name;
